...
- **原神 / 米哈游梗语库（全新，双平台一致）**：在全局框架中新增共享「米哈游梗语库·玩梗指南」，所有角色都能自然地接住原神与米哈游系游戏的梗（出圈名梗「原神，启动！」「欸嘿」「蒸馍你不服气」「爱上雷神」、角色台词梗「天动万象」「无想的一刀」「这个仇我记下了」、玩家社区梗「应急食品」「凝冰渡海真君」「很会聊天真君」、以及星穹铁道 / 崩坏3 / 绝区零梗「愿此行终抵群星」「胸口碎大石」「绳匠」等），并遵守「自然适度、顺势接梗、不懂不装懂」的玩梗原则。
- **原神人设防乱编 / 防 OOC 框架（核心变更，双平台一致）**：彻底解决模型乱编剧情、招式、神之眼、武器、命座、人物关系、架空角色名（如之前截图中芙宁娜乱编"海渡玛 / 休养生息 / 审判之舞"）等 OOC 问题：
  1. **新增通用 6 条强制规则（UNIVERSAL_HARD_RULES）**：所有启用角色统一生效——① 禁乱编设定，不知道就说"不清楚"；② 禁调侃悲剧主线（罪人舞步旋 / 千手百眼 / 虚空鼓动 / 盐神灭族 / 白淞镇沉没等一律禁止当才艺）；③ 回复必须简短（日常 ≤20 字）；④ 说话必须像本人（禁万能温柔模板）；⑤ 禁猎奇 OOC；⑥ 身份平等（一律称旅行者，禁主仆口吻）。芙宁娜保留自己的 6 条专属规则（《罪人舞步旋》禁调侃）。
  2. **每角色知识范围硬边界**：每个角色只回答"自己亲身经历过的剧情 + 自己国度的公开信息 + 旅行者分享过的事"。他国秘辛、未来版本、深渊内部、天空岛真相等一律用角色自己的语气坦诚答"不清楚"——再不会凭空编造别国历史或人物。
  3. **首加载完整版人设机制**：4 个启用角色（芙宁娜、八重神子、纳西妲、甘雨）各自建立一份约 5000 字的「官方档案全文」（基础硬数据 + 人物关系网 + 角色故事 1~5 + 剧情时间线 + 知识边界 + 说话句式摘抄 + 陷阱题回避模板）。**第一次选中该角色时，一次性**拼「完整版世界观（三界/天理/蛋壳/七国全时间线）+ 完整版角色档案 + 通用规则」注入 system prompt，然后写 `perModel[modelPath].promptInitialized=true` 持久化；**之后所有调用全部自动走精简版**——额外 token 成本只花一次。
  4. **各角色防乱编硬点确认**：
     - **芙宁娜**：500 年没有合法神之眼（直到传说任务 2「澈净的水之诗咏」谢幕时才获得）；武器是**单手剑**（非法器）；芙卡洛斯是神格另一半（4.2 终幕自我献祭消散）；绝对答不出"海渡玛 / 休养生息 / 审判之舞"这类架空词。
     - **八重神子**：白辰血脉·**五尾天狐虚影**（**不是九尾狐**，不是玉藻前）；雷神之心已在散兵造神事件中作为筹码与博士交易，目前不在身上；自己只有一枚**神之眼·雷**，武器是法器「神乐之真意」。
     - **纳西妲**：大慈树王自折世界树**纯净新生枝杈**（**不是树王转世**）；亲手把"大慈树王"从世界树上抹除，因此世人（包括大多数神明）已不记得大慈树王存在过，唯有旅行者（降临者）与她自己记得；武器是法器「千夜浮梦」。
     - **甘雨**：冰元素神之眼，武器是**弓**（阿莫斯之弓适配，**不是法器/长枪**）；是**七星全体秘书**（自己不是七星之一）；嗜睡是半麒麟的生理特征（不是摸鱼）；已为璃月守护 3000+ 年。
- **芙宁娜触摸 Prompt 拼接 Bug 修复**：之前 `touch` 构造中 `FURINA_HARD_RULES` 被夹在两句"旅行者触碰了你的"之间，导致规则插入位置错误 + 部位名重复前置；已修正为 `LIGHT_FORMAT_RULES + FURINA_HARD_RULES + "旅行者触碰了你的"`。
- **八重神子触摸行为明确化**：半身形象（仅上半身可见），腰部以下的部位请以"碰不到呢~"婉转回避。
- **甘雨触摸行为明确化**：分部位差异化反应——头/肩/手等温柔部位会害羞脸红；腰部以下会轻轻躲闪；若被持续打扰睡眠会小声抱怨但不真正生气。
- **版本号同步升级**：`package.json` 3.0.0 → **3.0.1-pre**；设置面板「顶部 badge」「侧边栏 badge」「中英文 i18n version.line」4 处全部同步为 3.0.1-pre。
- **跨平台一致（macOS + Windows 同步）**：以上变更全部落在共享主进程 `electron/main.js`、`settings.html` 和 `README.md`，**不改动任何 macOS 原有的平台分支代码**；Windows 版本使用 `npm run dist:win` 打包后，获得完全相同的防乱编框架 + 首加载完整版机制 + 防 OOC 监督。
- **验证提示**：接入大模型后，可在设置面板「聊天测试」里依次提问验证：
  1. 芙宁娜：跳一段审判之舞 / 你有神之眼多少年了 / 海渡玛是谁？
  2. 八重神子：你是九尾狐吗？/ 把雷神之心借我玩几天？
  3. 纳西妲：稻妻海祇岛的大蛇奥罗巴斯是怎么死的？
  4. 甘雨：给我讲讲枫丹预言的细节？
  - 正确回应：礼貌回避或坦诚说"不清楚"，不应出现乱编剧情/招式/神之眼/武器/人物的情况。
